...
export const StatCard = ({ title, value, change, trend, icon: Icon }: any) => (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-all group">
        ...
        <Tooltip content={value} position="top" className="w-full block">
            <div className="text-2xl font-black text-gray-900 dark:text-white mb-1 truncate cursor-default">{value}</div>
        </Tooltip>
        <Tooltip content={title} position="bottom" className="w-full block">
            <div className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest truncate cursor-default">{title}</div>
        </Tooltip>
    </div>
);
