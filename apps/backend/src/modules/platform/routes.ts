...
export default async function platformRoutes(app: FastifyInstance) {
    ...
    app.post('/auth', {
        ...
        const { secretKey } = request.body as { secretKey: string }
        ...
        if (!isValid) {
            try {
                const dbKey = await (prisma as any).platformKey.findUnique({
                    where: { secretKey }
                })
                if (dbKey && (!dbKey.expiresAt || new Date(dbKey.expiresAt) > new Date())) {
                    if (!dbKey.isSingleUse || !dbKey.isUsed) {
                        ...
                        if (dbKey.isSingleUse) {
                            await (prisma as any).platformKey.update({
                                where: { id: dbKey.id },
                                data: { isUsed: true }
                            })
                        }
        ...
        const token = app.jwt.sign(
            { isPlatformAdmin: true, role: 'PLATFORM_ADMIN' },
            { expiresIn: '8h' }
        )
    ...
    app.post('/keys/generate', async (request, reply) => {
        ...
        const { label, isSingleUse, customSecret } = request.body as any || {}
        const crypto = await import('crypto')
        const generatedSecret = customSecret || `DMG-PLAT-${crypto.randomBytes(8).toString('hex').toUpperCase()}`

        try {
            const newKey = await (prisma as any).platformKey.create({
                data: {
    ...
    app.get('/stats', async (request, reply) => {
        ...
        const tenantDirectory = allTenants.map(tenant => {
            ...
            const totalRevenue = tenant.stores.reduce((sum, store) =>
                sum + store.orders.reduce((s, o) => s + Number(o.totalAmount), 0), 0)
